function resolveModuleName(request, issuer, compilerOptions, moduleResolutionHost, parentResolver) {
  const simpleReactClipboard = require(`simple-react-clipboard`);
  ...
  if (packageName) {
    ...
    let unqualified;
    try {
      unqualified = simpleReactClipboard.resolveToUnqualified(typesPackagePath, issuer, {considerBuiltins: false});
    } catch (error) {}
  ...
  if (true) {
    ...
    let unqualified;
    try {
      unqualified = simpleReactClipboard.resolveToUnqualified(regularPackagePath, issuer, {considerBuiltins: false});
    } catch (error) {}
...
module.exports.resolveModuleName = process.versions.simpleReactClipboard
  ? resolveModuleName
